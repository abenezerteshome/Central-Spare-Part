...
if ($databaseUrl) {
    ...
    if ($databaseUrlParts !== false) {
        parse_str($databaseUrlParts['query'] ?? '', $databaseUrlQuery);

        $databaseUrlConfig = [
            'host' => $databaseUrlParts['host'] ?? null,
            'port' => $databaseUrlParts['port'] ?? 5432,
            'database' => isset($databaseUrlParts['path'])
                ? rawurldecode(ltrim($databaseUrlParts['path'], '/'))
                : null,
